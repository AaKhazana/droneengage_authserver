...
function fn_handleServerInfo(p_cmd) {
    try {
        ...
        if (m_communicationServersList.hasOwnProperty(p_server.m_commServerGUID)) {
            ...
            if (v_srvInfo.m_server.m_isOnline != v_msg.m_isOnline) {   // isOnline Changed

                v_srvInfo.m_server = p_server;
                if (v_msg.m_isOnline == true) {  // server is online again
            ...
            else {   // no change .. update other parameters if any
...
function fn_commServerMessageHandler(p_conn_GUID, p_msg) {
    try {
        ...
        if (p_cmd.c === global.c_CONSTANTS.CONST_CS_CMD_INFO) {   // send once when connection established
            fn_handleServerInfo(p_cmd);
        }
        else if (p_cmd.c === global.c_CONSTANTS.CONST_CS_CMD_LOGIN_REQUEST) {   // send as reply to LOGIN REQUEST
