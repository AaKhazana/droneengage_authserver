...
function fn_newLoginCard(p_accountName, p_accessCode, p_actorType, p_group, p_app, p_version, p_extra, p_mustGCS, fn_callback, fn_error) {
    ...
    if (p_accessCode != null) {   // avoid typo happens in android autocomplete
        p_accessCode = p_accessCode.trim();
    }

    if ((p_actorType == null) || (p_actorType.length !== 1)) {   // avoid typo happens in android autocomplete
        if (fn_error != null) {
            fn_error();
        }
        return;
...
function fn_logout(p_sessionID, fn_Success, fn_error) {
    ...
    if (c_loginCard == null) {   // FAKE Session could be a hack
        // 404 Error
        fn_error();
    }
